#!/usr/bin/env python3
"""
Bridge Level 2 at N=5000: Physical Pauli-Jordan (Johnston link matrix).
Focused test: pp-wave eps=3 + Schwarzschild only, M=10.
"""
...
PPW_EPS = [1.0, 3.0]  # two eps: scaling check + strongest signal
...
def run_seed(seed_idx, geometry, eps=None, base_seed=9300000):
    ...
    t_curved = time.time() - t0
    print(f" {t_curved:.0f}s", end='', flush=True)

    # Spectral
    print(f"  eigen...", end='', flush=True)
    ...
    t_spec = time.time() - t0
    print(f" {t_spec:.0f}s", end='', flush=True)
...
if __name__ == '__main__':
    print("=" * 72)
    print(f"BRIDGE LEVEL 2 @ N={N}: Physical Pauli-Jordan (Johnston link matrix)")
    print(f"M={M_SEEDS}, T={T}, zeta={ZETA}")
    print(f"Sorkin-Yazdi threshold = {math.sqrt(N)/(4*math.pi):.2f}")
    print("=" * 72, flush=True)
    ...
    for eps in PPW_EPS:
        ...
        key = f'ppw_eps{eps}'
        print(f"\n{'='*60}")
        print(f"PP-WAVE eps={eps} (E^2={E2:.2f})")
        print(f"{'='*60}", flush=True)

        dg, cj = [], []
        seeds = []
        for si in range(M_SEEDS):
            res = run_seed(si, 'ppwave', eps=eps)
        ...
        print(f"\n  SUMMARY: dG={dg.mean():+.4f}±{dg.std(ddof=1)/math.sqrt(M_SEEDS):.4f} (t={t_stat:+.2f})"
              f"  CJ={cj.mean():.6f}  corr={corr:+.3f}", flush=True)

    # SCHWARZSCHILD
    print(f"\n{'='*60}")
    print(f"SCHWARZSCHILD M={SCH_M}, r0={SCH_R0}, E^2={SCH_E2:.4f}")
    print(f"{'='*60}", flush=True)
    ...
    print(f"\n  SUMMARY: dG={dg_s.mean():+.4f}±{dg_s.std(ddof=1)/math.sqrt(M_SEEDS):.4f} (t={t_stat_s:+.2f})"
          f"  CJ={cj_s.mean():.6f}  corr={corr_s:+.3f}", flush=True)

    # COMPARISON N=2000 vs N=5000
    print(f"\n{'='*72}")
    print("N-SCALING COMPARISON (pp-wave eps=3)")
    print(f"{'='*72}")
    print(f"  N=2000: dG=+0.666±0.253 (t=2.63), CJ=0.02542, corr=+0.43")
    ppw3 = results.get('ppw_eps3.0', results.get('ppw_eps3'))
    if ppw3:
        print(f"  N=5000: dG={ppw3['dG_mean']:+.3f}±{ppw3['dG_se']:.3f} "
              f"(t={ppw3['t_stat']:+.2f}), CJ={ppw3['CJ_mean']:.5f}, corr={ppw3['corr']:+.3f}")

    print(f"\nN-SCALING (Schwarzschild)")
    print(f"  N=2000: dG=-0.010±0.119 (t=-0.08), CJ=0.00186, corr=+0.11")
    print(f"  N=5000: dG={dg_s.mean():+.3f}±{dg_s.std(ddof=1)/math.sqrt(M_SEEDS):.3f} "
          f"(t={t_stat_s:+.2f}), CJ={cj_s.mean():.5f}, corr={corr_s:+.3f}")

    # Save
    outfile = 'analysis/fnd1_data/bridge_level2_n5000.json'
    for key in results:
