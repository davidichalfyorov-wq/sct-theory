# ruff: noqa: E402, I001
"""
Generate Russian-language publication-quality figures for the D^2-quantization chirality paper.
...
# ---------------------------------------------------------------------------
# Style setup — Times New Roman for Russian text
# ---------------------------------------------------------------------------
try:
...
def figure_block_structure(n: int = 16, seed: int = 42):
    ...
    im0 = axes[0].imshow(abs_matrix, cmap="inferno", interpolation="nearest",
                          aspect="equal")
    axes[0].set_title(r"$|\delta(D^2)|$ в киральном базисе", fontsize=9)
    axes[0].set_xlabel("Индекс столбца", fontsize=8)
    axes[0].set_ylabel("Индекс строки", fontsize=8)
    ...
    axes[1].set_title(r"$\log_{10}|\delta(D^2)|$ (внедиаг. $\sim 10^{-16}$)",
                       fontsize=9)
    axes[1].set_xlabel("Индекс столбца", fontsize=8)
    axes[1].set_ylabel("Индекс строки", fontsize=8)
    ...
    fig.tight_layout()
    outpath = FIGURES_DIR / "fig_block_structure_ru.pdf"
    fig.savefig(outpath)
    plt.close(fig)
...
def figure_violation_correlation(n: int = 16, n_points: int = 200, seed: int = 123):
    ...
    if np.sum(mask_fit) > 5:
        ...
        ax.plot(x_fit, y_fit, "k--", linewidth=1.0, alpha=0.6,
                 label=f"Линейная подгонка (наклон $\\approx {coeffs[1]:.2f}$)")

    ax.set_xlabel(r"$\|\{D, \gamma_5\}\| / \|D\|$", fontsize=10)
    ax.set_ylabel(r"$\|[\delta(D^2), \gamma_5]\| / \|\delta(D^2)\|$", fontsize=10)
    ax.set_title("Корреляция нарушения киральности", fontsize=10)
    ax.legend(fontsize=8, loc="upper left")
    plt.colorbar(scatter, ax=ax, label=r"$\|\{D,\gamma_5\}\|/\|D\|$",
                  shrink=0.8)

    fig.tight_layout()
    outpath = FIGURES_DIR / "fig_violation_correlation_ru.pdf"
    fig.savefig(outpath)
    plt.close(fig)
...
def figure_loop_independence(n: int = 8, max_L: int = 8,
    ...
    ax.semilogy(loop_orders, plot_violations, "ko-", markersize=6,
                 linewidth=1.2, label="Макс. $|pq|$-коэфф.")

    # Machine epsilon line
    ax.axhline(y=eps_machine, color="red", linestyle="--",
                linewidth=0.8, alpha=0.7, label=r"Машинное $\varepsilon$")

    # Shade the "numerically zero" region
    ax.axhspan(1e-18, 1e-10, color="green", alpha=0.08)
    ax.text(4.5, 2e-14, "Численно ноль", fontsize=8, ha="center",
             color="green", fontstyle="italic")

    ax.set_xlabel("Порядок петель $L$", fontsize=10)
    ax.set_ylabel(r"Макс. $\|pq$-перекрёстный член$\| / \|$CT$_L\|$",
                   fontsize=10)
    ax.set_title(r"Независимость от порядка петель ($N=8$, 50 реализаций/порядок)",
                  fontsize=10)
    ...
    fig.tight_layout()
    outpath = FIGURES_DIR / "fig_loop_independence_ru.pdf"
    fig.savefig(outpath)
    plt.close(fig)
...
def figure_spectral_truncation(n_values=None, n_trials: int = 50, seed: int = 789):
    ...
    ax.semilogy(n_values, chiral_violations, "go-", markersize=7,
                 linewidth=1.2, label=r"$\{D,\gamma_5\}=0$ (киральный)")
    ...
    ax.axhline(y=np.finfo(float).eps, color="gray", linestyle=":",
                linewidth=0.8, label=r"Машинное $\varepsilon$")

    ax.set_xlabel("Размер матрицы $N$", fontsize=10)
    ax.set_ylabel(r"Макс. $\|[\delta(D^2), \gamma_5]\| / \|\delta(D^2)\|$",
                   fontsize=10)
    ax.set_title("Сходимость спектрального обрезания (50 реализаций)",
                  fontsize=10)
    ax.legend(fontsize=8)
    ...
    fig.tight_layout()
    outpath = FIGURES_DIR / "fig_spectral_truncation_ru.pdf"
    fig.savefig(outpath)
    plt.close(fig)
...
# ===================================================================
def main():
    print("Generating Russian-language figures for D^2-quantization chirality paper...")
    print("=" * 60)
