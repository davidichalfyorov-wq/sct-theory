#!/usr/bin/env python3
"""A_align exact-expmap midpoint at N=5000, M=20 seeds.
The decisive same-N comparison: ppw exact vs Sch exact-expmap midpoint."""
...
if __name__ == "__main__":
    ...
    print(f"ppw: exact, Sch: exact-expmap midpoint + quadratic jet (comparison)", flush=True)
    print(flush=True)
    ...
    print(f"  Expmap improvement: {ratio_expmap/max(ratio_quad,1e-15):.2f}x over quadratic", flush=True)
    ...
    with open(os.path.join(out_dir, "aalign_expmap_n5000.json"), "w") as f:
        json.dump({
