...
\begin{remark}[Directional language on plots]
When the same function is plotted against the variable $x = -k^2/\Lambda^2$,
the continuation \eqref{eq:phi-lor-closed} lives on the \emph{left} branch
$x < 0$.  Statements such as ``$\varphi$ grows exponentially'' refer to the
limit $x \to -\infty$ (equivalently $|x| \to \infty$ on that branch), not to
motion toward increasing $x$ along the horizontal axis.
\end{remark}
