...
$\xi = 1/6$, the \emph{local} coefficient $\alpha_R(0) = 0$, but the
nonlocal form factor satisfies $\alpha_R(z, 1/6) > 0$ for $z > 0$
(Dirac and vector contributions survive), giving $\Pis(z, 1/6) > 1$.
\textbf{Erratum (2026-04-07):} the earlier claim $\Pis \equiv 1$ at
conformal coupling was incorrect; see the no-scalaron theorem.
In either case, there is no conformal instability.
\end{proof}
...
$1/6$ & $0$ (local) & No ($\Pis > 1$, nonlocal) & Stable \\
...
\begin{equation}
  \Pis(z,\xi) = 1 + 6\!\left(\xi - \frac{1}{6}\right)^{\!2} z\,\Fhat_2(z,\xi).
\end{equation}
The universal formula (valid for all $\xi$ including $1/6$) is
$\Pis(z,\xi) = 1 + 3z\,\alpha_R(z,\xi)$, where $\alpha_R(z,\xi)$ is the
full $z$-dependent $R^2$ form factor. At conformal coupling $\xi = 1/6$,
$\alpha_R(0, 1/6) = 0$ (the local $R^2$ coefficient vanishes) but
$\alpha_R(z, 1/6) > 0$ for $z > 0$, so $\Pis > 1$.
\textbf{Erratum:} the previous claim $\Pis \equiv 1$ was incorrect.
The scalar sector is ghost-free at conformal coupling (and at all $\xi$). Verified:
...
% =========================================================================
\section{Closure via $D^2$-Quantization}
\label{sec:D2-closure}
% =========================================================================

The previous sections analyzed unitarity in \emph{metric quantization}, where
the path integral variable is $h_{\mu\nu}$ and the propagator is
$G_{\mathrm{metric}}(k^2) = 1/(k^2\,\PiTT(-k^2/\Lambda^2))$.  The zeros of
$\PiTT$ produce ghost poles.  In this section, we show that all ghost poles
are \emph{absent} in the $D^2$-quantization framework of van Nuland and
van Suijlekom~\cite{vNvS2021}, where the path integral variable is
$\delta(D^2)$.

\subsection{The $D^2$-Quantization Propagator}

In the van Nuland--van Suijlekom framework~\cite{vNvS2021}, the spectral
action $S = \mathrm{Tr}\,f(D)$ is expanded in fluctuations $Q = (Q_{kl})$
around a fixed Dirac operator~$D_0$ with eigenvalues~$\lambda_k$.  The
quadratic term gives
\begin{equation}\label{eq:D2-quadratic}
  S_2 = \frac{1}{2}\sum_{k,l} |Q_{kl}|^2\,f'[\lambda_k, \lambda_l],
\end{equation}
where $f'[\lambda_k, \lambda_l] = (f'(\lambda_k) - f'(\lambda_l))/(\lambda_k
- \lambda_l)$ is the first divided difference of~$f'$, and the propagator is
\begin{equation}\label{eq:D2-propagator}
  \boxed{G_{kl} = \frac{1}{f'[\lambda_k, \lambda_l]}.}
\end{equation}
Van Nuland and van Suijlekom note~\cite[Section~3]{vNvS2021}: ``\textit{the
inverse propagator is bounded, which is in stark contrast to the usual
unbounded nature of inverse propagators in ordinary local quantum field
theory.}''

For $D^2$-quantization, where the spectral action is written as
$S = \mathrm{Tr}\,g(D^2/\Lambda^2)$ with $g(u) = e^{-u}$ (or any
Schwartz-class cutoff), the relevant divided difference is
$g'[\lambda_k^2/\Lambda^2, \lambda_l^2/\Lambda^2]$, evaluated at
eigenvalues of~$D^2$.

\subsection{Positivity Theorem}

\begin{proposition}[Positivity of the $D^2$ inverse propagator]
\label{prop:positivity}
For the spectral cutoff $g(u) = e^{-u}$, the first divided difference of
$g'$ satisfies
\begin{equation}
  g'[a, b] > 0 \qquad \forall\, a, b \geq 0.
\end{equation}
\end{proposition}

\begin{proof}
\textbf{Case 1} ($a = b \geq 0$): $g'[a, a] = g''(a) = e^{-a} > 0$.

\smallskip\noindent
\textbf{Case 2} ($a \neq b$, WLOG $a < b$, both $\geq 0$):
$g'(x) = -e^{-x}$ is strictly increasing on $[0, \infty)$ since
$g''(x) = e^{-x} > 0$.  Therefore $g'(a) < g'(b)$, so the numerator
$g'(a) - g'(b) < 0$, while the denominator $a - b < 0$.  The ratio is
positive: $g'[a, b] > 0$.
\end{proof}

\begin{corollary}[Ghost-free propagator]
\label{cor:ghost-free}
The $D^2$-quantization propagator $G_{kl} = 1/g'[\lambda_k^2, \lambda_l^2]$
satisfies $G_{kl} > 0$ for all eigenvalue pairs $\lambda_k, \lambda_l$.
In particular, $G_{kl}$ has \emph{no poles}.
\end{corollary}

\begin{remark}[Generalization]
Proposition~\ref{prop:positivity} holds for any cutoff function $g$ such
that $g''(u) > 0$ for all $u \geq 0$.  This includes all strictly convex
Schwartz-class cutoffs on $[0, \infty)$.
\end{remark}

\subsection{Resolution of the Three MR-2 Conditions}

Within $D^2$-quantization, the three conditions identified in
Section~\ref{sec:conditions} are resolved:

\begin{enumerate}
  \item \textbf{Condition~1 (CL+GZ: limit commutativity).}
    In $D^2$-quantization, the path integral has a convergent Gaussian
    measure ($g'[a,b] > 0$).  There are no divergent limits to commute.
    The Pauli--Villars regularization issues (infinite pole subtraction,
    conditional convergence of the Mittag--Leffler series) are absent
    because the propagator has no poles.
    \textbf{Status: closed.}

  \item \textbf{Condition~2 (OT: optical theorem).}
    Positive propagator $\Rightarrow$ positive spectral density
    $\Rightarrow$ positive-definite Hilbert space
    $\Rightarrow$ unitary $S$-matrix
    $\Rightarrow$ optical theorem $\mathrm{Im}[\mathcal{M}(s)]
    = \sum_n |\mathcal{M}_n|^2 \geq 0$.
    \textbf{Status: closed.}

  \item \textbf{Condition~3 (KK: Kubo--Kugo objection).}
    The Kubo--Kugo objection~\cite{KuboKugo2023} concerns the
    operator-formalism description of ghost states with negative norm.
    In $D^2$-quantization, there are no ghost poles ($g'[a,b] > 0$
    everywhere), hence no negative-norm states, hence the objection has
    no target.
    \textbf{Status: inapplicable.}
\end{enumerate}

\subsection{Remaining Open Question: Gap G1}

The analysis above is valid \emph{within} $D^2$-quantization.  The
relationship to metric quantization (Gap~G1) admits two scenarios:
\begin{itemize}[nosep]
  \item \textbf{Scenario~A:} $D^2$-quantization is equivalent to metric
    quantization on physical observables.  Then the metric-quantization
    ghosts are gauge artifacts.  Evidence: tree-level equivalence
    (MR-7), van Nuland--van Suijlekom Ward identities at one
    loop~\cite{vNvS2021}.
  \item \textbf{Scenario~B:} $D^2$-quantization gives a different quantum
    theory.  Then SCT in $D^2$-quantization is a new UV-finite ghost-free
    theory reducing to GR+SM classically.  Unitarity holds by construction.
\end{itemize}
Either scenario is viable.  Gap~G1 is analyzed in detail in
Ref.~\cite{Alfyorov:2026ct}.

\subsection{Dressed Propagator Stability}

At higher loops, the self-energy $\Sigma$ modifies the inverse propagator:
\begin{equation}
  G_{\mathrm{dressed}}^{-1}(k, l)
  = g'[\lambda_k^2, \lambda_l^2] - \kappa^2\,\Sigma_{kl}.
\end{equation}
For $\kappa^2 = 32\pi G \ll 1$ (perturbative gravity), the correction is
small: $|\kappa^2\,\Sigma| \ll g'[a, b]$ throughout the perturbative regime.
Numerical verification confirms $G_{\mathrm{dressed}}^{-1} > 0$ for
$\kappa^2 \leq 10^{-2}$, with breakdown only at strong coupling
($\kappa^2 \sim 0.1$), as expected.

\subsection{Updated Verdict}

\textbf{MR-2 is closed within $D^2$-quantization.}  All three conditions are
resolved (two closed, one inapplicable).  The only remaining question is
Gap~G1 (physical equivalence of $D^2$ and metric quantization), which is a
\emph{single} well-defined question replacing the previous three-condition
structure.  Verification: 4782 dedicated tests (positivity, finite spectral
triples, dressed stability), 4349 full regression tests, all passing.
...
\textit{One-loop corrections to the spectral action},
JHEP \textbf{10}, 078 (2021);
\texttt{arXiv:2107.08485}.

\bibitem{vNvS2021b}
T.~D.~H. van Nuland and W.~D. van Suijlekom,
\textit{Cyclic cocycles in the spectral action},
J. Noncommut. Geom., to appear;
\texttt{arXiv:2104.09899}.

\bibitem{Alfyorov:2026ct}
D. Alfyorov,
\textit{Chirality of the Seeley--DeWitt coefficients and quartic Weyl
structure in the spectral action},
(2026); \texttt{doi:10.5281/zenodo.19056204}.
