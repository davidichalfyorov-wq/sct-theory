...
is the imaginary error function.  Although \eqref{eq:phi-erfi} contains the
explicit factor $e^{-z/4}$, the full function does \emph{not} decay
exponentially on the positive real axis: the large-$z$ asymptotic
$\operatorname{erfi}(\sqrt{z}/2) \sim 2e^{z/4}/\sqrt{\pi z}$ cancels that
factor, leaving only the power-law behaviour $\varphi(z) \sim 2/z$.
