...
\begin{itemize}[nosep]
  ...
  \item At $\xi = 1/6$, the \emph{local} $R^2$ coefficient
    $\alpha_R(0, 1/6) = 0$ vanishes, but $\Pi_{\mathrm{s}}(z, 1/6) > 1$
    for $z > 0$ because the nonlocal form factor
    $\alpha_R(z, 1/6) \neq 0$ (fermion and vector contributions).
    \textbf{Erratum (2026-04-07):} the earlier claim
    $\Pi_{\mathrm{s}} \equiv 1$ at $\xi = 1/6$ was based on the
    factorized form $6(\xi-1/6)^2 \hat{F}_2$, which has a $0/0$
    indeterminacy at conformal coupling. The correct universal formula
    $\Pi_s = 1 + 3z\,\alpha_R(z,\xi)$ resolves this and gives
    $\Pi_s > 1$.
...
\begin{figure}[ht]
  ...
  \caption{The scalar propagator denominator $\Pi_{\mathrm{s}}(z,\xi)$
    ...
    At conformal coupling $\xi = 1/6$, the \emph{local}
    $R^2$ coefficient vanishes, but the nonlocal form factor
    $\alpha_R(z, 1/6)$ remains positive for $z > 0$
    (Dirac and vector contributions survive). Hence
    $\Pi_{\mathrm{s}}(z, 1/6) > 1$ for all $z > 0$.
    For all $\xi$, $\Pi_{\mathrm{s}}$ has no positive real zero:
...
\end{itemize}
All consistent with the Phase~3 verified values.
