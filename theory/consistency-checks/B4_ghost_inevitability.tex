...
For $z < 0$, $\varphi(z) = \int_0^1 e^{|z|\xi(1-\xi)}\,\dd\xi$
grows as $\varphi(z) \sim e^{|z|/4}\sqrt{2\pi/|z|}$ (dominated by
the saddle point at $\xi = 1/2$).  The form factors $h_C^{(s)}(z)$
inherit this exponential growth, so $F_1(z)$ grows exponentially for
