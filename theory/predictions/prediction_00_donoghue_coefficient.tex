...
% ============================================================
% SCT Theory — Benchmark 00: Donoghue Coefficient
% Status: VERIFIED (by construction — correspondence principle)
% Classification: Consistency benchmark, NOT a distinguishing prediction.
% Any UV completion reproducing Einstein--Hilbert in the IR yields the same result.
% ============================================================
...
\title{%
  \textbf{SCT Consistency Benchmark B-00:}\\[6pt]
  \Large Donoghue Quantum Correction to Newtonian Potential\\[4pt]
  \large $\beta = \dfrac{41}{10\pi}$\\[4pt]
  \normalsize\textit{(Model-independent --- not a distinguishing SCT prediction)}
}
...
\begin{center}
  \colorbox{verified!15}{\parbox{0.7\textwidth}{\centering
    \textbf{\color{verified} STATUS: VERIFIED (Consistency Benchmark)}\\[2pt]
    \small By construction --- correspondence principle.\\
    \small This is model-independent: any UV completion with an EH IR limit yields the same $\beta$.
  }}
\end{center}
...
\colorbox{verified!15}{\parbox{0.8\textwidth}{\centering
  \textbf{\color{verified}\Large VERIFIED --- CONSISTENCY BENCHMARK}\\[6pt]
  ...
  \textbf{Classification:} Necessary consistency condition, not a distinguishing prediction.\\
  This benchmark confirms SCT passes the correspondence principle but does not\\
  differentiate it from other UV completions (string theory, asymptotic safety, LQG, etc.).
}}
\end{center}
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{P-01 (Fixed $c_1/c_2$ ratio):} While $\beta$ is model-independent, the higher-derivative Wilson coefficients $c_1, c_2$ are \emph{not}. The general formula $c_1/c_2 = -1/3 + 120(\xi - 1/6)^2/13$ is a genuinely distinguishing SCT prediction, beyond what the correspondence principle guarantees. At conformal coupling $\xi = 1/6$, this reduces to $c_1/c_2 = -1/3$.
