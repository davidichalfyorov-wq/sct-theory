...
(Ricci-scalar-squared), whose local limits yield the SM-determined coefficient
...
(iii)~at conformal coupling $\xi = 1/6$, the \emph{local} $R^2$ coefficient
vanishes ($\alpha_R = 0$), though the nonlocal form factor remains nonzero
for nonzero momenta;
...
entire functions of $\Box/\Lam^2$. The propagator denominators
$\Pi_{\mathrm{TT}}$ and $\Pi_s$ are also entire, with zeros that
correspond to massive modes (spin-2 fakeon and, for some $\xi$,
scalar Lee--Wick resonances).  In a second companion paper~\cite{Alfyorov:2026ss}
...
\begin{equation}\label{eq:stelle-eom}
  ...
  = \frac{1}{2}\,T_{\mu\nu},
\end{equation}
with the same structure as the full equation~\eqref{eq:full-eom}
but with $\hat{F}_i \to F_i(0)$ and $\Theta^{(C,R)} = 0$.
This is the Stelle higher-derivative gravity field
equation~\cite{Stelle:1978} in the conventions of~\eqref{eq:full-eom},
with the SM-derived coefficients
$\alpha_C = 13/120$ and $\alpha_R(\xi) = 2(\xi - 1/6)^2$.
Note: in~\eqref{eq:stelle-eom} the coefficients are
$2\alpha_C/(16\pi^2) = 2F_1(0)$ and $\alpha_R/(16\pi^2) = F_2(0)$,
consistent with the $1/(16\pi^2)$ prefactor in~\eqref{eq:S-quad}.
...
$-R/\kappa^2 - 6\alpha_R\,\Box R/(16\pi^2) = T/2$, consistent with the Stelle trace
equation~\cite{Stelle:1976gc} in the conventions of~\eqref{eq:stelle-eom}.
...
amplitudes $-4/3$ and $+1/3$ are determined entirely by the
...
\begin{equation}\label{eq:V-short}
  ...
  \quad\text{as } r \to 0,
\end{equation}
in the local Yukawa (Stelle) approximation with both modes present.
\emph{Note:} when the no-scalaron theorem applies (i.e., no spin-0 mode),
the $+\frac{1}{3}$ term is absent and $V(0)/V_\mathrm{N} = -1/3$
instead~\cite{sct_no_scalaron}.
Both values are artifacts of the local (Yukawa) approximation;
the full nonlocal potential $V(0)/V_\mathrm{N}$ diverges at one-loop order
because $1/\PiTT(z) - 1 \to -1$ as $z \to \infty$, making the Fourier
integral $\int [1/\PiTT - 1]\,dk$ divergent at $r = 0$.
At large distances ($r \gg 1/\Lam$), both Yukawa terms decay
exponentially and Newton's law is recovered; in this regime the local
approximation is valid.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\textwidth]{nt4a_newtonian_potential.pdf}
  \caption{Modified Newtonian potential $V(r)/V_\mathrm{N}(r)$ as a function
  of $r\,\Lam$ in the local Yukawa approximation~\eqref{eq:V-ratio}.
  In the local Stelle approximation (both modes present), the potential
  vanishes at the origin ($V(0) = 0$); see text for caveats.  It recovers Newton's law
  at large distances ($V \to V_\mathrm{N}$), and features an intermediate
  repulsive regime from the spin-2 Yukawa correction.
  Dashed lines show the individual spin-2 ($-\frac{4}{3}e^{-m_2 r}$) and
  spin-0 ($+\frac{1}{3}e^{-m_0 r}$) contributions.}
  \label{fig:newtonian-potential}
\end{figure}

\subsection{Observational bounds}

Confrontation with gravitational experiments yields a lower
bound on the spectral cutoff.  The dedicated bound from the
no-scalaron analysis~\cite{Alfyorov:2026ns}, using the single-Yukawa
(spin-2 only) potential with the nonlocal mass $m_2 = 1.554\,\Lam$,
gives
\begin{equation}\label{eq:Lambda-bound}
  \Lam > 3.53 \times 10^{-3}\;\mathrm{eV}
  \qquad (1/\Lam < 55.9\;\mu\mathrm{m}),
\end{equation}
from the E\"ot-Wash torsion-balance experiment.
Earlier estimates using the double-Yukawa (Stelle) parametrization
with local masses gave weaker bounds ($\Lam > 2.565$~meV from
Eöt-Wash, $\Lam > 2.38$~meV from Cassini); these are superseded
by~\eqref{eq:Lambda-bound}.  All post-Newtonian parameters satisfy
experimental constraints with exponential margin~\cite{Will:2014}.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\textwidth]{ppn1_exclusion.pdf}
  \caption{Exclusion plot for the spectral cutoff $\Lam$ from
  solar-system and laboratory experiments.  The shaded region is excluded
  at $95\%$ CL.  The tightest bound
  $\Lam > 2.565 \times 10^{-3}\;\mathrm{eV}$ comes from the E\"ot-Wash
  torsion-balance experiment; the Cassini time-delay measurement gives
  $\Lam > 2.38 \times 10^{-3}\;\mathrm{eV}$.  See
  Paper~II~\cite{Alfyorov:2026ss} for details.}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\textwidth]{nt4b/nt4b_effective_masses.pdf}
  \caption{Effective masses $m_2/\Lam$ (spin-2, solid) and $m_0(\xi)/\Lam$
  (spin-0, dashed) as functions of the non-minimal coupling~$\xi$.
  The spin-2 mass $m_2 = \Lam\sqrt{60/13} \approx 2.148\,\Lam$ is
  $\xi$-independent.  The spin-0 mass diverges at conformal coupling
  $\xi = 1/6$ (scalar mode decoupling).  Both masses satisfy the Higuchi
  bound $m > \frac{3}{2}H_0$ by many orders of magnitude for all
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\textwidth]{mt2/mt2_suppression.pdf}
  \caption{Fractional Hubble correction $\delta H^2/H^2$ as a function of
  redshift~$z$ for $\xi = 0$ and $\Lam = \Lam_{\mathrm{PPN}}
  = 2.38 \times 10^{-3}\;\mathrm{eV}$.  The correction is below
  $10^{-55}$ at all epochs, confirming the structural suppression of
  cosmological modifications.  The peak near $z \sim 3400$ (matter--radiation
  equality) reflects the maximum of $H(z)/\Lam$.}
...
The Cauchy problem for nonlocal field equations of the form~\eqref{eq:full-eom}
requires a careful specification of initial data, since the infinite-order
operator $F(\Box/\Lam^2)$ formally requires infinitely many initial
conditions.  Calcagni, Modesto, and Nardelli~\cite{Calcagni:2018gke}
showed that for actions built from entire form factors, the physical
degrees of freedom remain finite: the auxiliary initial data are
determined by regularity conditions and by the requirement that the
solution converges to the GR solution in the infrared limit.  In the SCT
context, the entireness of $F_1$ and $F_2$ (established in Paper~I)
ensures that the field equations are free of run-away solutions associated
with spurious poles, and the Cauchy problem is well-posed in the
perturbative sense around any solution of the Einstein equations.
...
The implication is clear: nonlocal gravity theories with entire-function form factors (SCT, IDG)
...
\end{align}
The minus sign in front of $N_D$ reflects the fermionic factor
$(-1)^{2s+1} = -1$ for $s = 1/2$, arising from the Grassmann nature of
the fermion determinant; the individual $\beta_W^{(1/2)} = 1/20$
is positive.  Numerically: $4/120 - 135/120 + 144/120 = 13/120$.
