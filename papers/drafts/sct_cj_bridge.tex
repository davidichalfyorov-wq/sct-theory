...
Direct measurement shows, however, that the past--future
correlation is partial:
$\langle a_- a_+ \rangle / \langle a_-^2 \rangle
= 0.512 \pm 0.016$ ($N = 3000$, $\varepsilon = 5$, $M = 20$),
while the symmetry
$\langle a_+^2 \rangle / \langle a_-^2 \rangle = 0.949$ is
approximately satisfied.  At this correlation level, the naive
prediction would be
$\langle (a_- + a_+)^2 \rangle \approx 3.0 \langle a^2 \rangle$
rather than~$4$.  Nevertheless, the full formula with factor~$4$
matches the Monte Carlo data ($R = 1.016 \pm 0.015$), while a
factor of~$3$ would give $R \approx 1.35$.

The resolution is that the CJ estimator operates in logarithmic
space ($Y = \log_2$), where the relationship between the
fractional path-count response~$a_\pm$ and the effective
covariance kernel is nonlinear.  The factor~$4$ is confirmed
empirically but its derivation from the properties of the Hasse
diagram remains an open problem.  In non-vacuum spacetimes
($R \neq 0$), the $s \leftrightarrow 1-s$ symmetry is broken,
and the factor may receive corrections of order $R T^2$.
\end{remark}
