...
($\alpha = 0.955 \pm 0.027$, consistent with $8/9$ at the
...
\begin{equation}\label{eq:alpha-fit}
  \alpha = 0.955 \pm 0.027\,,
\end{equation}
which lies $2.4\sigma$ above $8/9 = 0.889$ but is also consistent
with $\alpha = 1$ at the $1.7\sigma$ level.  The Bayesian information criterion (BIC) favours the $8/9$
model over the free-exponent fit despite the slightly lower
residual RMS of the latter ($3.6\%$ vs $3.8\%$), because the
penalty for the additional parameter outweighs the marginal
improvement in fit quality.  The exponent depends on the fitting range:
$\alpha = 0.87$ for $N \in [500, 3000]$, $0.96$ for $[500, 10000]$,
and $1.00$ for $[5000, 10000]$.  An alternative model
$\CJ \propto N/(\log N)^{0.39}$ is equally consistent with the data.
...
($\alpha = 0.955$).  Both are consistent with data.}
\label{fig:nscaling}
\end{figure}
...
($h = 2xy$), which has the same $\Esq = \varepsilon^2/2$, using
two causal predicates:
\begin{itemize}
\item \textbf{Exact predicate} (leading-order Synge world function):
  the ratio $\CJ_{\mathrm{cross}}/\CJ_{\mathrm{plus}}
  = 0.98 \pm 0.13$ at $N = 2000$, $\varepsilon = 5$, $M = 15$
  seeds.  The two polarisations give \emph{equal} CJ within
  statistical errors.
\item \textbf{Jet predicate} (second-order RNC approximation):
  the ratio is $3.0 \pm 0.2$ at $N = 2000$, $\varepsilon = 3$,
  $M = 20$ seeds.
\end{itemize}
The discrepancy is an artifact of the jet predicate, which
introduces an anisotropic systematic error that couples differently
to the two polarisation alignments.  With the exact predicate, CJ
is polarisation-independent at fixed~$\Esq$, consistent with the
isotropic angular average of
...
This confirms that the Poisson sprinkling effectively performs the
angular averaging over the transverse sphere.
...
with the exact causal predicate, plus- and cross-polarised pp-waves
at the same $\Esq$ give equal CJ values
($\CJ_{\mathrm{cross}}/\CJ_{\mathrm{plus}} = 0.98 \pm 0.13$).
The previously reported factor-of-$3$ discrepancy was an artifact
of the jet (RNC) predicate.
\end{remark}
...
\begin{remark}[General-$d$ structure]
\label{rem:general-d}
...
\item Characterisation of the jet-predicate artifacts that produce
  a spurious polarisation dependence
  (Section~\ref{sec:polarisation}).  The factor-of-3 polarisation
