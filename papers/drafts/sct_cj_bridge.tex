...
below~$4\%$.  Eight independent diagnostic tests are reported,
including a de~Sitter null test ($\CJ = 0$ exactly), Kottler
cross-term, polarisation independence, a Lorentz boost test
confirming $\CJ \propto E^2$ (blind to the magnetic Weyl
component~$B^2$, match $< 1\%$), and Sorkin--Johnston entropy
...
invariants~\cite{Surya:2019ndm,Dowker:2013,Henson:2006}.
...
established in~\cite{BenincasaDowker:2010,Belenchia:2015,Glaser:2018}.
...
in the context of entanglement entropy.  The causal set
d'Alembertian and its continuum limit have been further developed
in~\cite{Dowker:2010,Aslanbeigi:2014}, while the spectral
dimension of causal sets has been investigated
in~\cite{Eichhorn:2019}.
...
describes eight diagnostic tests.  Section~\ref{sec:derivation}
...
of independent seeds.  A Shapiro--Wilk test on $M = 50$ trials at
$N = 2000$ confirms normality ($W = 0.988$, $p = 0.88$); the
bootstrap 95\% confidence interval for~$R$ is $[0.89, 1.09]$.}
\label{tab:ratio}
\begin{tabular}{rrccc}
\toprule
...
\subsection{$T^4$ scaling}
\label{sec:tscaling}

The bridge formula predicts $\CJ \propto T^4$.  We verify this at
$N = 3000$, $\varepsilon = 5$, $M = 15$ seeds over four values
of~$T$:

\begin{center}
\begin{tabular}{rcccc}
\toprule
$T$ & $\CJ_{\mathrm{meas}}$ & $\CJ_{\mathrm{pred}}$
    & $R$ & $\CJ/T^4$ \\
\midrule
0.5 & $6.50 \times 10^{-3}$ & $6.21 \times 10^{-3}$ & 1.05 & $1.04 \times 10^{-1}$ \\
0.7 & $2.55 \times 10^{-2}$ & $2.38 \times 10^{-2}$ & 1.07 & $1.06 \times 10^{-1}$ \\
1.0 & $9.74 \times 10^{-2}$ & $9.93 \times 10^{-2}$ & 0.98 & $9.74 \times 10^{-2}$ \\
1.5 & $4.60 \times 10^{-1}$ & $5.03 \times 10^{-1}$ & 0.92 & $9.09 \times 10^{-2}$ \\
\bottomrule
\end{tabular}
\end{center}

A power-law fit $\CJ \propto T^{\alpha_T}$ yields
$\alpha_T = 3.86$, consistent with the predicted~$4.0$ within the
...
An independent extended scan at six values $\varepsilon \in
\{0.5, 1, 2, 3, 5, 10\}$ covering two orders of magnitude in~$E^2$
confirms this result: a weighted-mean fit gives $\chi^2/\mathrm{dof}
= 0.48$ (5~degrees of freedom, $M = 15$ seeds per point),
consistent with a constant~$R_\varepsilon$.
...
We present eight independent tests that constrain the structure
...
\subsection{Lorentz boost test: $E^2$ vs $B^2$ discrimination}
\label{sec:boost}

On pp-wave spacetimes, $E^2 = B^2 = \varepsilon^2/2$, so one
cannot discriminate between $\CJ \propto E^2$,
$\CJ \propto E^2 + B^2$, and $\CJ \propto C^2/8 = E^2 - B^2$
from pp-wave data alone.  To break this degeneracy, we apply a
Lorentz boost along the transverse ($z$) axis to a Schwarzschild
background.  A transverse boost with velocity~$v$ mixes the
electric and magnetic parts of the Weyl tensor while preserving
the invariant $C^2 = 8(E^2 - B^2)$.

The three competing predictions are:
\begin{center}
\begin{tabular}{lccc}
\toprule
$v$ & $E^2/E^2_0$ & $(E^2{+}B^2)/(E^2{+}B^2)_0$ & $C^2/C^2_0$ \\
\midrule
0.0 & 1.000 & 1.000 & 1.000 \\
0.3 & 1.326 & 1.652 & 1.000 \\
0.5 & 2.333 & 3.667 & 1.000 \\
\bottomrule
\end{tabular}
\end{center}

We compute CJ using the full Riemann jet predicate (including
$R_{0ijk}$ magnetic and $R_{ijkl}$ spatial contributions) at
$M = 0.05$, $r_0 = 0.50$, $N = 2000$, $T = 1$, $M = 20$~seeds:

\begin{center}
\begin{tabular}{lcccc}
\toprule
$v$ & $\CJ/\CJ_0$ (data) & $E^2/E^2_0$ & $(E^2{+}B^2)$-pred.\ & $C^2$-pred.\ \\
\midrule
0.0 & 1.000 & 1.000 & 1.000 & 1.000 \\
0.3 & $1.334 \pm 0.19$ & 1.326 & 1.652 & 1.000 \\
0.5 & $2.311 \pm 0.34$ & 2.333 & 3.667 & 1.000 \\
\bottomrule
\end{tabular}
\end{center}

The data match the $E^2$ prediction to better than~$1\%$ at both
boost velocities ($0.6\%$ at $v = 0.3$, $0.9\%$ at $v = 0.5$).
The superenergy prediction $E^2 + B^2$ is rejected at the
$19\%$~level ($v = 0.3$) and $37\%$~level ($v = 0.5$).  The
Lorentz-invariant $C^2$ is rejected at the $33\%$~and $131\%$
levels respectively.

\textbf{CJ is proportional to~$\Esq$ and is blind to the magnetic
Weyl component~$B^2$.}  This confirms that the covering relation
(Hasse diagram) provides access to the observer-projected electric
Weyl invariant, not to the frame-independent Weyl scalar.  The
$B^2$-blindness is a structural property of the nilpotent
adjacency algebra, not a limitation of the estimator design.
...
\begin{remark}[Two-leg factor $4 = 2_{\mathrm{alg}} \times 2_{\mathrm{dyn}}$]
\label{rem:factor4}
The CJ observable involves the squared response of the link score
$Y = \log_2(p^{\downarrow} p^{\uparrow} + 1)$.  In the bulk
regime ($p^{\downarrow} p^{\uparrow} \gg 1$), $Y$ decomposes as
$Y \approx \log_2 p^{\downarrow} + \log_2 p^{\uparrow}$, and
the variation splits into past and future components,
$\delta Y \approx g_- + g_+$ where
$g_- = \delta \log_2 p^{\downarrow}$ and
$g_+ = \delta \log_2 p^{\uparrow}$.

\medskip\noindent\textbf{Symmetric-channel decomposition.}\enspace
Define the symmetric and antisymmetric combinations
\begin{equation}\label{eq:gs-ga}
  g_s = \frac{g_- + g_+}{\sqrt{2}}\,,\qquad
  g_a = \frac{g_- - g_+}{\sqrt{2}}\,.
\end{equation}
Since $\delta Y = \sqrt{2}\,g_s$, the squared response is
$\delta Y^2 = 2\,g_s^2$.  Thus CJ depends \emph{only} on the
symmetric channel~$g_s$ and is completely blind to the
antisymmetric channel~$g_a$.

Denote the stratified covariances
$M_{--} = Q_N(g_-^2)$,\, $M_{++} = Q_N(g_+^2)$,\,
$M_{-+} = Q_N(g_- g_+)$,\, where
$Q_N[F] = \sum_B w_B \Cov_B(|X|, F)$.  Then
\begin{equation}\label{eq:Mss-def}
  M_{ss} = \tfrac{1}{2}(M_{--} + 2M_{-+} + M_{++})\,,\quad
  M_{aa} = \tfrac{1}{2}(M_{--} - 2M_{-+} + M_{++})\,,
\end{equation}
and $\CJ = 2\,M_{ss}$.  The factor~$4$ in $C_0 = 4\,C_{\mathrm{an}}$
therefore decomposes as
\begin{equation}\label{eq:factor4-decomp}
  4 = \underbrace{2}_{\text{algebraic}} \times
      \underbrace{\frac{M_{ss}}{C_{\mathrm{an}} \cdot N^{8/9} E^2 T^4}}_{\to\, 2\text{ (dynamical)}}\,.
\end{equation}
The first factor is a trivial algebraic identity from $\delta Y^2 = 2\,g_s^2$.
The second factor, $M_{ss} / C_{\mathrm{an}}\text{-norm} \to 2$, is a
dynamical statement about Hasse-path statistics that we verify numerically
but do not derive analytically.

\medskip\noindent\textbf{Numerical verification.}\enspace
Table~\ref{tab:symmetric} presents the symmetric-channel decomposition
at six values of~$N$ ($M = 30$ CRN trials each, $\varepsilon = 3$).
\begin{table}[ht]
\centering
\caption{Symmetric-channel decomposition of CJ, normalised by
$C_{\mathrm{an}} \cdot N^{8/9} E^2 T^4$.}
\label{tab:symmetric}
\begin{tabular}{rccccc}
\toprule
$N$ & $M_{ss}$ & $M_{aa}$ & $M_{sa}$ & $R_{\mathrm{full}}$ & $M_{--} \approx M_{++}$? \\
\midrule
1\,000  & 2.008 & 0.898 & 0.100  & 1.00 & $1.55,\; 1.35$ \\
3\,000  & 1.974 & 0.592 & 0.162  & 0.99 & $1.44,\; 1.12$ \\
5\,000  & 2.011 & 0.565 & 0.067  & 1.01 & $1.35,\; 1.22$ \\
8\,000  & 2.018 & 0.539 & 0.056  & 1.01 & $1.33,\; 1.22$ \\
10\,000 & 2.059 & 0.522 & $-0.013$ & 1.03 & $1.28,\; 1.30$ \\
\bottomrule
\end{tabular}
\end{table}
...
\item $M_{ss} \to 2$ as $N \to \infty$ (the ``dynamical~$2$''),
  confirming the factor~$4$.
\item $M_{sa} \to 0$: the past--future symmetry is restored, so
  $M_{--} \to M_{++}$ in the continuum limit.
\item $M_{aa} \to 0.52 \neq 0$: the antisymmetric channel carries
  a persistent nonzero signal, but CJ is blind to it.
\end{enumerate}
The ``equalisation'' hypothesis $M_{--} = M_{-+} = M_{++}$
(which would require $M_{aa} = 0$) is definitively excluded.

\medskip\noindent\textbf{One-leg CJ is negative.}\enspace
A ``pure one-leg'' CJ using one-leg weight
$|X_{\mathrm{down}}| = |\log_2 p^{\downarrow}
- \overline{\log_2 p^{\downarrow}}|$ and one-leg response
$g_-^2$ is \emph{negative} at all tested~$N$
($\CJ_{\text{1-leg}} \approx -3.4 \times 10^{-3}$ at
$N = 3000$, $M = 25$).  The product structure
$p^{\downarrow} p^{\uparrow}$ in the link score is therefore
not merely an enhancement --- it is constitutive of the
observable's ability to detect curvature.  Without the two-leg
product, the covariance changes sign and the bridge formula
fails entirely.

\medskip\noindent\textbf{Score dependence.}\enspace
The coefficient~$C_0$ is specific to the logarithmic link score.
Replacing $Y = \log_2(p^{\downarrow} p^{\uparrow} + 1)$ with
$(p^{\downarrow} p^{\uparrow})^{1/4}$ changes the effective
coefficient by a factor of~${\sim}\,7$ at $N = 3000$, confirming
that $C_0$ is a property of the observable, not a universal
geometric invariant.

\medskip\noindent\textbf{Indivisibility.}\enspace
The coefficient $C_0 = 32\pi^2/(3 \cdot 9! \cdot 45)$ should
be regarded as an \emph{indivisible} property of the
two-leg stratified covariance.  The arithmetic factorisation
$4 = 2_{\mathrm{alg}} \times 2_{\mathrm{dyn}}$ identifies the
origin of the factor but does not reduce it to independently
derived components: the dynamical~$2$ (i.e.\ $M_{ss} \to
2\,C_{\mathrm{an}}$) is an empirical fact about Hasse-path
statistics on Poisson causal sets that requires a theory of
stratified covariance on random DAGs for its derivation
...
factor $4 = 2_{\mathrm{alg}} \times 2_{\mathrm{dyn}}$ is
characterised in Remark~\ref{rem:factor4}.  The $N^{8/9}$ exponent is
...
accompanying this paper~\cite{Alfyorov:2026repo}, in the directory
...
= 1.042 \pm 0.053$), consistent with the isotropic angular average.
The Lorentz boost test (Section~\ref{sec:boost}) directly
discriminates between $E^2$, $E^2 + B^2$, and $C^2$: the data
match $\CJ \propto E^2$ to $< 1\%$, rejecting both superenergy
and Lorentz-invariant alternatives.  CJ is blind to the magnetic
Weyl component~$B^2$.
...
\item Construction of a complementary observable sensitive to the
  magnetic Weyl component~$B^2$.  The Lorentz boost test
  (Section~\ref{sec:boost}) confirms that CJ is blind to~$B^2$;
  recovery of the full invariant $C^2 = 8(E^2 - B^2)$ requires
  an observable built from the full causal order rather than the
  covering relation alone.
\item Analytical derivation of the dynamical factor~$2$
  in~\eqref{eq:factor4-decomp}: prove that $M_{ss} \to
  2\,C_{\mathrm{an}}$ as $N \to \infty$.  This requires a
  theory of stratified covariance on Poisson DAGs
  (Remark~\ref{rem:factor4}).
\end{enumerate}
...
\item Kottler shows a $22\%$ Ricci reduction relative to
  Schwarzschild.
\item Lorentz boost test confirms $\CJ \propto E^2$, blind
  to~$B^2$ (match $< 1\%$ at $v = 0.3$ and $v = 0.5$).
