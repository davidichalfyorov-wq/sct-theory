...
\begin{remark}[Two-leg factor $4 = 2^2$]
\label{rem:factor4}
...
the variation is $\delta Y \approx (a_- + a_+)/\ln 2$, where
$a_- = \delta p^{\downarrow}/p^{\downarrow}$ and
$a_+ = \delta p^{\uparrow}/p^{\uparrow}$.  If the past and
future responses were perfectly correlated
($\langle a_- a_+ \rangle = \langle a_-^2 \rangle$), the
two-leg structure would yield
$\langle (a_- + a_+)^2 \rangle = 4 \langle a^2 \rangle$ and
hence $C_0 = 4\, C_{\mathrm{an}}$.

Direct measurement reveals the internal structure of the
factor~$4$.  The past--future correlation is partial:
$\langle a_- a_+ \rangle / \langle a_-^2 \rangle
= 0.51 \pm 0.02$ (stable across $N$ and~$\varepsilon$), while
the symmetry $\langle a_+^2 \rangle / \langle a_-^2 \rangle
\approx 0.95$ is approximately satisfied.  A direct comparison
of two-leg CJ to one-leg CJ (where only $\delta(\log_2
p^{\downarrow})$ is used as the response) gives:
\begin{center}
\begin{tabular}{cc}
\toprule
Component & Value \\
\midrule
$\CJ_{\mathrm{2\text{-}leg}} / \CJ_{\mathrm{1\text{-}leg}}$ & $2.8$--$3.1$ (stable across $K$, $N$, $\varepsilon$) \\
$\CJ_{\mathrm{1\text{-}leg}} / C_{\mathrm{an}}\text{-prediction}$ & $1.1$--$1.5$ (varies with $N$) \\
Product & $\approx 4 \pm 10\%$ \\
\bottomrule
\end{tabular}
\end{center}
The two-leg enhancement is $\approx 3$ (not~$4$), reflecting the
partial correlation $\rho \approx 0.5$.  The one-leg CJ exceeds
the analytical prediction $C_{\mathrm{an}} = 8\pi^2/(3 \cdot 9!
\cdot 45)$ by $\approx 30\%$, due to the nonlinear relationship
between the logarithmic weight~$|X|$ and the theoretical
kernel~$k_4(s)\,k_4(1{-}s)$.  The product of these two effects
gives $3.07 \times 1.29 \approx 3.97 \approx 4$.

Crucially, a ``pure one-leg'' CJ (using one-leg weight
$|X_{\mathrm{down}}| = |\log_2 p^{\downarrow}
- \overline{\log_2 p^{\downarrow}}|$ and one-leg response
$\delta Y_{\mathrm{down}}^2$) is \emph{negative}
($-4.4 \times 10^{-3}$ at $N = 3000$).  The product structure
...
product, the covariance with $\delta Y^2$ changes sign and the
bridge formula fails.

The coefficient $C_0 = 32\pi^2/(3 \cdot 9! \cdot 45)$ should
therefore be regarded as an \emph{indivisible} property of the
two-leg stratified covariance, not as a product of a ``two-leg
factor'' and a ``one-leg coefficient.''  The arithmetic
decomposition $32 = 4 \times 8$ is trivially true, but does
not correspond to a physical factorisation into single-leg
components.

In non-vacuum spacetimes ($R \neq 0$), the
$s \leftrightarrow 1-s$ symmetry is broken, and the factor may
receive corrections of order~$RT^2$.
\end{remark}
