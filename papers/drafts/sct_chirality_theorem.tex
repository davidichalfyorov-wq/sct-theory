...
\small Independent researcher, Russia\\[2pt]
...
$\sigma^{rs} = \frac{1}{4}[\gamma^r, \gamma^s]$ commute with the chirality
...
ratio condition.  We show that the strict spectral renormalizability conjecture
for gravity is \emph{excluded at the level of structural analysis}: the graviton propagator is chirality-blind
(a scalar on the traceless-transverse sector), and multi-eigenvalue sums at two
loops and above generate $pq$ cross-terms outside the spectral function space.
The precise obstruction is the multi-trace structure of divided differences in
the van~Nuland--van~Suijlekom framework applied to gravity, where the metric
modifies the Dirac operator itself rather than entering as an inner fluctuation.
However, the failure is quantitatively negligible: the cross-terms are
suppressed by $\alpha_C^3/(16\pi^2)^3 \approx 3.2 \times 10^{-10}$, and the
spectral action provides effective UV finiteness within the perturbative regime
to this precision.  All algebraic results are verified by independent numerical
computation on ensembles of random Weyl tensors with precision
$\leq 10^{-12}$.
\end{abstract}
...
The chiral decomposition of the spin connection in four dimensions is
a classical result in differential geometry and mathematical physics.
Eguchi, Gilkey, and Hanson~\cite{EguchiGilkeyHanson:1980} established
the block-diagonal structure of the spin curvature in their comprehensive
review of gravitational instantons.  The same decomposition underlies
the Atiyah--Singer index theorem in four dimensions and has been used
extensively in self-dual gravity and instanton
physics~\cite{AtiyahSinger:1968,AlvarezGaume:1983ig}.  In the spectral
action context, Chamseddine, Connes, and
Marcolli~\cite{ChamseddineConnesMarcolli:2007} computed the full Standard
Model action from the spectral action principle on the product geometry
$M \times F$, and van~Suijlekom~\cite{vanSuijlekom:2014pya} provided a
systematic treatment of the heat kernel expansion.

The novelty of the present paper lies not in the chirality of the spin
generators (which is classical) but in the systematic application of this
property to the spectral action's quartic Weyl structure.  We show that the
chiral block-diagonal decomposition, combined with the Cayley--Hamilton
identity for the Weyl spinors, reduces the space of quartic Weyl invariants
from three (Molien count) to one effective structure, exactly matching the
single spectral parameter.  This resolves the apparent overdetermination of
the three-loop finiteness problem.

The one-loop form factors of the spectral action with full Standard Model
content --- yielding $\alpha_C = 13/120$ and
$\alpha_R(\xi) = 2(\xi - 1/6)^2$ --- have been computed
in~\cite{Alfyorov:2026ff}, and the observational consequences (lower bound
$\Lam > 2.565 \times 10^{-3}\;\mathrm{eV}$) are derived
in~\cite{Alfyorov:2026ss}.
...
the three quartic $\Omega$-structures numerically.
...
\begin{remark}[Scope of the Ricci-flat restriction]
\label{rem:ricci-flat}
The block-diagonal property $[\Omega_{\mu\nu}, \gamma_5] = 0$
(Theorem~\ref{thm:chirality}) and the commutation
$[D^2, \gamma_5] = 0$ hold on \emph{any} four-dimensional background,
not just Ricci-flat ones: the Lichnerowicz scalar curvature term $R/4$
is proportional to the identity and commutes with $\gamma_5$ trivially.
The Ricci-flat restriction is needed only for the $p$-$q$ decomposition of
the heat kernel coefficients, because on a general background the Riemann
tensor includes Ricci contributions that do not separate into self-dual
and anti-self-dual Weyl sectors.  On a general Einstein manifold
($R_{\mu\nu} = \lambda g_{\mu\nu}$), the heat kernel retains the chiral
block-diagonal structure, but the coefficients $\tr(a_{2n})$ contain
additional terms involving $\lambda$ that couple to both chiral sectors
symmetrically.  The zero-$pq$ theorem (Theorem~\ref{thm:zero-pq}) remains
valid for the Weyl-dependent part of $\tr(a_{2n})$, but the total
coefficient receives Ricci-squared and mixed Ricci--Weyl contributions that
require separate treatment.  Extension to general backgrounds is an
important open problem for the full three-loop program.
\end{remark}
...
\end{equation}
in the $2 \times 2$ block gives, after
...
The heat kernel coefficient $a_8$ on a Ricci-flat manifold involves three
...
\begin{alignat}{2}
  ...
  \label{eq:S3}
\end{alignat}
Note that the cubic structure
$C_{\mu\nu\rho\sigma}\,\tr(\Omega^{\mu\nu}\Omega^{\rho\sigma})$
is of mass dimension~6 (cubic in curvature), and hence does not appear
at the quartic (dimension~8) level.
...
The structures $S_1$ and $S_2$ are individually block-diagonal at
...
Equation~\eqref{eq:cancel} is a non-trivial \emph{prediction} of the
chirality theorem, not a circular consequence: it constrains the
relationship between the spinor-sector and geometry-sector Avramidi
coefficients~\cite{Avramidi:1999bm} and can be independently verified by
computing $c_{S_3}$ and $\beta_{\mathrm{geom}}$ from the Avramidi
recursion.  We have confirmed the cancellation numerically: for $10^3$
random Weyl tensors, the total $pq$ coefficient of $\tr(a_8)$ is
$|\beta| < 10^{-12}$ (see Table~\ref{tab:omega} and
Section~\ref{sec:numerical}, test~(9)).
...
\begin{tikzpicture}[
  ...
  \node[box, fill=blue!8] (A) at (0,0) {3 invariants\\$K_1,\,K_2,\,K_3$};
  \node[box, fill=orange!10] (B) at (5,0) {2 invariants\\$K_1,\,K_3$};
  \node[box, fill=green!10] (C) at (10,0) {1 structure\\$p^2 + q^2$};
  \draw[arr] (A) -- (B) node[lbl] {Cayley--Hamilton};
  \draw[arr] (B) -- (C) node[lbl] {Chirality ($\beta=0$)};
\end{tikzpicture}
\caption{Reduction chain for quartic Weyl invariants in the spectral action.
The Cayley--Hamilton identity for traceless $3 \times 3$ Weyl spinors
reduces three Molien invariants to two; the chirality theorem further
reduces them to one effective structure, matching the single spectral
parameter~$f_8$.}
...
\begin{conjecture}[Spectral renormalizability for gravity --- strict form]
\label{conj:spectral}
...
\end{equation}
for some spectral function $g_L$ depending on the loop order.
\end{conjecture}

\noindent\textbf{Status: excluded by structural argument and numerical evidence.}
The strict conjecture is excluded at the level of structural analysis.  The precise
obstruction is that the multi-loop effective action involves multi-eigenvalue
sums (divided differences in the van~Nuland--van~Suijlekom
framework~\cite{vanNuland:2021}) whose propagator structure does not respect
the chiral block decomposition.  The graviton propagator
$G^{\mathrm{TT}}(k) = [k^2\,\Pi_{\mathrm{TT}}(k^2/\Lam^2)]^{-1}$
is a \emph{scalar} on the traceless-transverse sector: it acts identically
on the $(\mathbf{3}, \mathbf{1})$ and $(\mathbf{1}, \mathbf{3})$ components
under $\mathrm{SU}(2)_L \times \mathrm{SU}(2)_R$ and therefore connects
$\delta C^+$ to $\delta C^-$ perturbations.  Consequently, multi-eigenvalue
sums at two loops and above generate $pq$ cross-terms between the self-dual
and anti-self-dual Weyl sectors that cannot be absorbed by any spectral
function deformation.

This has been verified numerically: in a toy spectral triple with
$D^2 = D^2_L \oplus D^2_R$, the mixed partial derivative
$\partial^2\Gamma_2/\partial p\,\partial q \neq 0$ to high precision, and
the cross-term fraction remains at $\sim 70$--$85\%$ as the spectrum size
grows from $N = 4$ to $N = 32$.

The fundamental difference from the Yang--Mills case is that the gauge field
enters the Dirac operator as an \emph{inner fluctuation}
$D \to D + A + JAJ^*$, preserving the spectral structure, whereas the
graviton modifies the operator \emph{itself}: $D = D[g]$.

\begin{conjecture}[Practical spectral UV-finiteness]
\label{conj:practical}
All $pq$ cross-terms in the gravitational counterterms are suppressed by at
least $\alpha_C^3/(16\pi^2)^3 \approx 3.2 \times 10^{-10}$ relative to
the tree-level action, where $\alpha_C = 13/120$ is the one-loop Weyl
coefficient.  Within the perturbative regime (loop orders
$L \leq L_{\rm opt}$, where the indicative $L_{\rm opt} \sim (16\pi^2/\alpha_C)^{1/2}
\approx 78$ is the estimated loop order at which the perturbative expansion parameter
$[\alpha_C/(16\pi^2)]^L$ reaches order unity), the spectral action provides
an effective absorption of all divergences to precision $\sim 10^{-10}$.
\end{conjecture}

\noindent\textbf{Supporting evidence for the practical statement.}
\begin{enumerate}[(a)]
  \item \textbf{Chirality theorem (proven, this paper).}
    All single-trace spectral functionals $\Tr\,g(D^2/\Lam^2)$ have
    identically zero $pq$ cross-terms (Theorem~\ref{thm:chirality}).
    This applies to the spectral action's own Seeley--DeWitt expansion
    at all orders in the curvature.

  \item \textbf{One-loop finiteness (proven).}
    The one-loop counterterm $\tr(a_4)$ is a single-trace spectral
    invariant with the form $\alpha_C C^2 + \alpha_R R^2$, absorbed
    by spectral function deformation.  $D = 0$ unconditionally at
    one loop.

  \item \textbf{Two-loop on-shell finiteness (proven).}
    ...
    a deformation of $f_6$.  The $pq$ question does not arise at dimension~6.

  \item \textbf{Three-loop suppression.}
    The first loop order where $pq$ cross-terms are physically relevant is
    $L = 3$ (dimension~8 counterterms).  The multi-trace contribution
    carrying $pq$ enters at $\order(\alpha_C^3/(16\pi^2)^3) \approx
    3.2 \times 10^{-10}$, negligible for all physical purposes.

  \item \textbf{Yang--Mills case (proven).}
    Van~Nuland and van~Suijlekom~\cite{vanNuland:2021} proved one-loop
    spectral renormalizability for Yang--Mills, showing that the framework is
    sound when the gauge field enters as an inner fluctuation.  The gravity
    case fails because the metric modifies the operator itself.
\end{enumerate}

\begin{remark}
The strict conjecture is excluded at the level of structural analysis, but the failure is quantitatively
negligible.  The spectral action retains its one-parameter predictive
structure through at least two loops (unconditionally on-shell) and provides
effective UV finiteness within the perturbative regime to precision
$\sim 10^{-10}$.  This places the spectral action in a category distinct
from both standard GR (non-renormalizable at two loops) and Stelle's
quadratic gravity~\cite{Stelle:1976gc} (renormalizable but with ghosts):
the spectral action is \emph{practically} UV-finite with a precisely
quantified failure threshold.
\end{remark}
...
\begin{enumerate}[(1)]
  ...
  \item \textbf{$[\sigma^{rs}, \gamma_5] = 0$} (12 tests).
    ...
    in both the $\frac{1}{4}[\gamma^r, \gamma^s]$ and
    $\frac{1}{2}[\gamma^r, \gamma^s]$ conventions.  Maximum commutator
    norm $< 10^{-15}$.
  ...
  \item \textbf{Quartic $\Omega$-structures} (4 tests).
    $S_1$, $S_2$, $S_3$, and $C^2 \cdot \Omega^2$ all verified
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.7\textwidth]{../../analysis/figures/chirality/fig_pq_convergence.pdf}
\caption{Convergence of the fitted $pq$ cross-term coefficient $\beta$ to zero
with increasing ensemble size.  For each ensemble of $N$ random Weyl tensors,
$\tr((\Omega^2)^2)$ is computed and fit to $\alpha(p^2 + q^2) + \beta\,pq$.
The coefficient $\beta$ is consistent with zero at machine precision
($|\beta| < 10^{-16}$) for all ensemble sizes, confirming the chirality
theorem prediction.  Error bars denote the least-squares standard error
of the fit.}
...
The remaining question---whether multi-loop counterterms respect the spectral
action structure---is resolved in the negative by structural argument
and numerical evidence on finite spectral triples.  The strict spectral
renormalizability conjecture (Conjecture~\ref{conj:spectral}) is
\emph{excluded at the level of structural analysis}: the graviton propagator is chirality-blind (a scalar on the
traceless-transverse sector), and multi-eigenvalue sums at two loops and above
generate $pq$ cross-terms that lie outside the spectral function space.
This obstruction is fundamental to gravity and does not arise in the
Yang--Mills case~\cite{vanNuland:2021}, where the gauge field enters as an
inner fluctuation preserving spectral structure.

However, the practical spectral UV-finiteness statement
(Conjecture~\ref{conj:practical}) survives with strong quantitative support:
the $pq$ cross-terms are suppressed by $\alpha_C^3/(16\pi^2)^3 \approx
3.2 \times 10^{-10}$ relative to the leading action, and the first loop order
at which they become physically relevant is $L = 3$ (dimension~8
counterterms), since the unique $C^3$ structure at dimension~6 does not
distinguish $pq$ from $p^2 + q^2$.

The results of this paper provide the algebraic foundation for understanding
the three-loop structure of the spectral action and precisely quantify both the
scope and the limits of spectral protection against UV divergences.  The key
implications for the broader program are: (1)~the spectral action's built-in
chirality reduces the effective dimensionality of the counterterm space from
two to one at the level of the heat kernel expansion, exactly matching the
single available spectral parameter; (2)~this reduction does not extend to
multi-loop counterterms due to the chirality-blindness of the graviton
propagator; (3)~the failure is quantitatively negligible, with the spectral
action providing effective UV finiteness to precision $\sim 10^{-10}$.
...
All symbolic results were verified numerically to $\geq 12$-digit
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{../../analysis/figures/chirality/fig_crossed_chirality.pdf}
\caption{Crossed chirality assignment verified on 200 random Weyl tensors.
Left: $\tr_R(\Omega^2)$ depends linearly on $p = |C^+|^2$ with zero
residual in~$q$.
Right: $\tr_L(\Omega^2)$ depends linearly on $q = |C^-|^2$ with zero
residual in~$p$.  The slopes are $-1/2$ in each chiral
sector, as predicted by equation~\eqref{eq:trOsq}.}
...
\toprule
Degree in $W$ & 0 & 2 & 3 & 4 & 5 & 6 & 7 & 8 & 9 & 10 \\
\midrule
$P(\text{deg})$ & 1 & 1 & 1 & 3 & 2 & 7 & 5 & 13 & 12 & 23 \\
\bottomrule
\end{tabular}
\end{center}
...
In the loop-order correspondence (mass dimension $= 2L + 2$ for an $L$-loop
on-shell counterterm in Ricci-flat gravity, hence degree $L + 1$ in the
Weyl tensor), $L = 3$ corresponds to degree~4, giving 3 invariants
versus 1 spectral parameter.  After
