...
for gravity is \emph{disproven}: the graviton propagator is chirality-blind
...
the SM-determined Weyl-squared coefficient $\alpha_C = 13/120$ and an $R^2$
...
\noindent\textbf{Status: disproven.}
The strict conjecture is disproven at the structural level.  The precise
...
coefficient.  Within the perturbative regime (loop orders $L \leq L_{\rm opt}
\sim 78$), the spectral action provides an effective absorption of all
divergences to precision $\sim 10^{-10}$.
\end{conjecture}
...
\begin{remark}
The strict conjecture is disproven, but the failure is quantitatively
...
action structure---is resolved in the negative.  The strict spectral
renormalizability conjecture (Conjecture~\ref{conj:spectral}) is
\emph{disproven}: the graviton propagator is chirality-blind (a scalar on the
