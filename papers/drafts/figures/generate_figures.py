# ruff: noqa: E402, I001
"""
Publication figures for 'Nonlocal one-loop form factors of the spectral
action with Standard Model content'.

Generates 5 PDF figures in papers/drafts/figures/:
  1. fig_phi.pdf          -- Master function phi(x)
  2. fig_hC.pdf           -- Weyl form factors h_C^{(s)}(x), s = 0, 1/2, 1
  3. fig_hR.pdf           -- R^2 form factors h_R^{(s)}(x)
  4. fig_alpha_total.pdf   -- SM totals alpha_C(x) and alpha_R(x, xi)
  5. fig_potential.pdf     -- Modified Newtonian potential V(r)/V_N(r)

Execute: python papers/drafts/figures/generate_figures.py
"""
...
def figure_alpha_total():
    ...
    ax1.set_title(r'(a) Weyl coefficient', fontsize=9)
    ...
    ax2.set_title(r'(b) $R^2$ coefficient', fontsize=9)

    fig.tight_layout(w_pad=2.5)

    path = FIGURES_DIR / "fig_alpha_total.pdf"
    fig.savefig(str(path), bbox_inches="tight")
    plt.close(fig)
    print(f"  [OK] {path.name}")
    return path
...
def figure_potential():
    ...
    path = FIGURES_DIR / "fig_potential.pdf"
    fig.savefig(str(path), bbox_inches="tight")
    plt.close(fig)
    print(f"  [OK] {path.name}")
    return path
...
def main():
    print("=" * 60)
    print("SCT Form Factors — Publication Figures")
    print("=" * 60)
    ...
    print(f"\nGenerated {len(paths)} figures in {FIGURES_DIR}/")
    for p in paths:
        print(f"  {p.name}")
    return 0
