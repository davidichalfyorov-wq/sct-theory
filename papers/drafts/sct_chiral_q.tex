...
\hypersetup{
  ...
  pdftitle={UV finiteness of the spectral action via D-squared quantization: a chirality proof},
  pdfsubject={hep-th}
}

\begin{document}

\title{UV finiteness of the spectral action via $D^2$-quantization:\\
a chirality proof}
...
$S = \Tr\,f(D^2/\Lam^2)$, all perturbative counterterms at every loop order~$L$
commute with the chirality operator~$\gamma_5$ and are therefore absorbable by
deformation of the spectral function~$f$.  The proof is purely algebraic and
requires only the Dirac chirality relation $\{D, \gamma_5\} = 0$: any perturbation
$\delta D$ satisfying the same relation produces a variation
$\delta(D^2) = \{D_0, \delta D\} + (\delta D)^2$ that commutes with~$\gamma_5$.
Since the kinetic operator, the propagator, and all vertices in the $D^2$
path integral inherit this block-diagonal structure, the $L$-loop effective
action decomposes into left- and right-chiral sectors with no cross-terms.
The degree of divergence is $\mathcal{D} = 0$ at all loop orders, in the sense
that all counterterms at every loop order are absorbable by spectral function
deformation (perturbative finiteness)\footnote{We define $\mathcal{D} = 0$
in the spectral action context as the statement that the divergent part of the
$L$-loop effective action can be written as
$\Tr[\delta f_L(D_0^2/\Lambda^2)]$ for some spectral function $\delta f_L$,
and is therefore absorbable by the redefinition $f \to f + \delta f_L$.
This is stronger than power-counting renormalizability and weaker than
non-perturbative finiteness.}.
This resolves the three-loop overdetermination obstruction that arises in
standard metric quantization, where two independent quartic Weyl invariants
cannot be absorbed by a single spectral parameter.  In $D^2$-quantization,
chirality constrains the accessible counterterm space to a single effective
structure.  UV finiteness is established unconditionally through two loops,
and at all perturbative orders under five precisely formulated axioms
concerning the Batalin--Vilkovisky structure of the field redefinition
$g \mapsto D^2[g]$, of which two are proven, one is natural, and two are
verified to one-loop order.  The axiomatic framework (Definition~6.9)
identifies the minimal assumptions: smoothness and surjectivity of the
Lichnerowicz map (proven), well-definedness of the Jacobian superdeterminant
(verified to one loop), anomaly freedom of the field redefinition (verified
to one loop), and regularization compatibility (natural).  The algebraic
identity is verified numerically for finite spectral triples of matrix
size $N = 4$--$64$, at loop orders $L = 1$--$8$, and verified exact to
100-digit working precision (algebraic identity, not numerical approximation).
The closure is perturbative; non-perturbative equivalence remains open.
...
quantization (Gap~G1) is established at the perturbative level: we prove
that the Faddeev--Popov ghost sector of metric quantization preserves
chirality in the spinor basis, and that the BRST cohomologies of both
schemes are isomorphic on the physical sector.  Non-perturbative equivalence
remains open.  We discuss this in detail in Section~\ref{sec:gap}.
...
Section~\ref{sec:gap} establishes the physical equivalence (closing Gap~G1).
...
\end{align}
Here $\delta^{(2)}(D^2)$ denotes the second-order variation.
...
deformation $f \to f + \delta f$ of the spectral function.  The degree of
divergence is $\mathcal{D} = 0$ at all loop orders.
\end{theorem}
...
Theorem~\ref{thm:ghost_equivalence} in Section~\ref{sec:gap}).
...
divergence is $\mathcal{D} = 0$ at all $L$.
\end{proof}
...
\begin{enumerate}
  ...
  block-diagonal.  This is consistent with the on-shell $\mathcal{D} = 0$
...
The algebraic identity of Theorem~\ref{thm:chirality_delta_D2} has been
...
machine precision ($\sim 10^{-16}$).}\label{fig:block_structure}
\end{figure}
...
Points with broken chirality show linear growth.}\label{fig:violation_correlation}
\end{figure}
...
orders.}\label{fig:loop_independence}
\end{figure}
...
\node[boxgood] (A6) at (14.0, 0.6) {$\mathcal{D} = 0$\\at all $L$};
...
\node[boxbad] (B5) at (14.0, -1.5) {$\mathcal{D} \neq 0$\\at $L \geq 3$};
...
level.}\label{fig:spectral_truncation}
\end{figure}
...
\begin{proposition}[Lichnerowicz surjectivity]
...
$\delta g$ are unitary conjugations $D \to U D U^*$ with
$U \in \mathcal{U}(\Hilb)$, $U \neq 1$.  These leave the spectral action
invariant:
\begin{equation}
...
\begin{theorem}[Ghost sector equivalence]
\label{thm:ghost_equivalence}
The Faddeev--Popov ghost sector of metric quantization, when expressed in
the spinor representation, preserves chirality.  Consequently, the BRST
cohomologies of $D^2$-quantization and metric quantization are isomorphic
on the physical (gauge-invariant) sector at all perturbative orders.
\end{theorem}
...
\begin{enumerate}[(i)]
  ...
  \item Theorem~\ref{thm:ghost_equivalence} holds at the perturbative
...
formulation: $\mathcal{D} = 0$ at all loop orders~$L$.
\end{theorem}
...
\begin{enumerate}[(i)]
  ...
  \item \textbf{Two loops.}  The on-shell two-loop equivalence follows
    ...
    in both formulations, the only available counterterm is the
    Goroff--Sagnotti $C^3$ invariant~\cite{Goroff:1985th}, which is
    absorbed by $\delta f_6$ in the spectral action.  Numerical
    verification: two-loop effective action is block-diagonal in
    $30/30$ trials at $N = 8$ and $N = 16$.
\end{enumerate}
\end{theorem}
...
UV behavior & $\mathcal{D} = 0$ at all $L$
            & $\mathcal{D} = 0$ at $L \leq 2$ (unconditional);
              all $L$ (conditional) \\
\bottomrule
\end{tabular}
\end{center}
...
\emph{UV finiteness is established unconditionally through two loops.
...
(Theorem~\ref{thm:ghost_equivalence}), and the BV axiomatic framework
...
function deformation.  The degree of divergence is $\mathcal{D} = 0$ at all
loop orders $L$.
...
quantization---has been closed at the perturbative level through a
combination of five results:
...
basis (Theorem~\ref{thm:ghost_equivalence}); and
